...
void HVH::DoDistortion() {
	...
	auto distortdelta = g_menu.main.antiaim.distortion_range.get();
	auto distortspeed = g_menu.main.antiaim.distortion_speed.get();
	if (g_cl.m_speed > 5.f && g_cl.m_ground && !GetKeyState(g_menu.main.misc.fakewalk.get())) {
...
void HVH::AntiAimPitch() {
	...
	switch (m_pitch) {
	case 1:
		if (aa_on_use && g_cl.m_cmd->m_buttons & IN_USE)
			return;
		else
		// down.
...
void HVH::GetAntiAimDirection() {
	...
	if (m_base_angle > 0) {
		if (m_base_angle == 1 || (aa_on_use && g_cl.m_cmd->m_buttons & IN_USE))
			m_view = 0.f;
...
void HVH::AntiAim() {
	...
	if (m_mode == AntiAimMode::STAND) {
		...
		aa_on_use = g_menu.main.antiaim.enable_use.get();
	}
