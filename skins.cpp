...
void Skins::think() {
	...
	const auto item_system_offset = *reinterpret_cast<std::int32_t*>(g_csgo.addr + 1);

	// Add the offset to the end of the instruction
	const auto item_system_fn = reinterpret_cast<CCStrike15ItemSystem * (*)()>(g_csgo.addr + 5 + item_system_offset);
	...
	const auto get_paint_kit_definition_offset = *reinterpret_cast<std::int32_t*>(g_csgo.addr + 11 + 1);

	// Add the offset to the end of the instruction
	const auto get_paint_kit_definition_fn = reinterpret_cast<CPaintKit * (__thiscall*)(CCStrike15ItemSchema*, int)>(g_csgo.addr + 11 + 5 + get_paint_kit_definition_offset);
	...
	for (int i{ 1 }; i <= g_csgo.m_entlist->GetHighestEntityIndex(); ++i) {
		...
		if (g_menu.main.skins.knife.get() > 0 && ent->is(HASH("CPredictedViewModel"))) {
			...
			Color backup_color0, backup_color1, backup_color2, backup_color3;

			if (g_menu.main.skins.custom_paintkit.get())
			{
...
void Skins::UpdateItem(Weapon* item) {
	...
	g_cl.m_updating_paintkit = false;
}
